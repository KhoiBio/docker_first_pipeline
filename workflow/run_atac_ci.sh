...
samtools view -b ${OUTPUT_DIR}/${SAMPLE}_dedup.bam chrFake > ${OUTPUT_DIR}/${SAMPLE}_nomt.bam
...
if ls ${OUTPUT_DIR}/${CONDITION}_*_final.bam 1> /dev/null 2>&1; then
    samtools merge ${OUTPUT_DIR}/${CONDITION}_merged.bam ${OUTPUT_DIR}/${CONDITION}_*_final.bam
    samtools sort -o ${OUTPUT_DIR}/${CONDITION}_merged.sorted.bam ${OUTPUT_DIR}/${CONDITION}_merged.bam
    samtools index ${OUTPUT_DIR}/${CONDITION}_merged.sorted.bam
    rm ${OUTPUT_DIR}/${CONDITION}_merged.bam
else
    echo "No final BAMs found for merge, creating empty BAM"
    samtools view -b -o ${OUTPUT_DIR}/${CONDITION}_merged.sorted.bam /dev/null
    samtools index ${OUTPUT_DIR}/${CONDITION}_merged.sorted.bam
fi

# Step 11: Convert to BEDPE
samtools sort -n -o ${OUTPUT_DIR}/${CONDITION}_merged.name_sorted.bam ${OUTPUT_DIR}/${CONDITION}_merged.sorted.bam
bedtools bamtobed -bedpe -i ${OUTPUT_DIR}/${CONDITION}_merged.name_sorted.bam > ${OUTPUT_DIR}/${CONDITION}_merged.bedpe || true

# Step 12: Shift Tn5 sites
awk -F $'\t' 'BEGIN {OFS = FS}{ if ($9 == "+") {$2 = $2 + 4; $6 = $6 - 5} else if ($9 == "-") {$3 = $3 - 5; $5 = $5 + 4} print $0}' \
    ${OUTPUT_DIR}/${CONDITION}_merged.bedpe > ${OUTPUT_DIR}/${CONDITION}_merged.shifted.bedpe || true

# Step 13: MACS2 peak calling
macs2 callpeak -t ${OUTPUT_DIR}/${CONDITION}_merged.shifted.bedpe \
    -f BEDPE -g ${GENOME_SIZE} -n ${CONDITION} \
    --outdir ${OUTPUT_DIR} --keep-dup all --call-summits || true
